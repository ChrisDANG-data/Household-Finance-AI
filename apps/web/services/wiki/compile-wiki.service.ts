...
function categoryNotePath(category: string): string {
  return `Categories/${wikiSlug(category)}`;
}
...
function buildIndexPage(input: {
  ...
  const lines = [
    ...
    "",
    "## Hub",
    `- ${wikiLink("Ledger/Overview")}`,
    `- ${wikiLink("Ledger/Events")}`,
    "",
    "## Documents",
  ...
  return { path: "Household Index.md", content: `${lines.join("\n")}\n` };
}
...
function buildOverviewPage(input: {
  ...
  const lines = [
    "# Ledger overview",
    "",
    `Back to ${wikiLink("Household Index")}.`,
    "",
    "## Balances",
    ...
    "",
    `See ${wikiLink("Ledger/Events")} for itemized events.`,
  ];

  return { path: "Ledger/Overview.md", content: `${lines.join("\n")}\n` };
}
...
function buildEventsIndexPage(eventLinks: string[]): WikiFile {
  const lines = [
    "# Ledger events",
    "",
    `Back to ${wikiLink("Household Index")} · ${wikiLink("Ledger/Overview")}`,
    "",
  ...
  return { path: "Ledger/Events.md", content: `${lines.join("\n")}\n` };
}
...
function buildEventPage(input: {
  ...
  const lines = [
    ...
    "",
    `Back to ${wikiLink("Ledger/Events")} · ${wikiLink(categoryNotePath(input.category))}`,
    "",
    "## Details",
...
function buildReadmePage(): WikiFile {
  return {
    ...
    content: [
      ...
      "",
      `Start at ${wikiLink("Household Index")}.`,
      "",
    ].join("\n"),
  };
}
